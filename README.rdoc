...
I happen to be using Heroku. Here's how you'd deploy there:
 ...
 git clone http://github.com/matthewtodd/kitchen.git
 cd kitchen
...
I've made a few patches: see config/init.rb, lib/openid/store/couchdb.rb, and slices/chef-server-slice. (I'm mid-process on my  http://wiki.opscode.com/display/opscode/Contributor+License+Agreement. It would be nice to whack the lib and slices directories, as well as the monkey patches in init.rb!) Notably, I've totally stubbed out Chef::Queue, so there's no search indexing. I don't know if this is a big deal yet.
